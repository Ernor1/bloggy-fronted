...
const CommentCard = ({ data, type, postPath }: any) => {
  ...
  return (
    <Card
      radius="sm"
      className={clsx(
        data.user.id === user?.id ? "border-default-400" : "",
        " border"
      )}
      shadow="none"
    >
      <CardHeader className="justify-between">
        <div className="flex items-center">
          <h5 className="font-semibold">{data.user.username}</h5>{" "}
          <Icon name="dot" strokeWidth={1.25} />
          <span className="text-sm">
            {moment(data.lastUpdatedOn, moment.ISO_8601).format("Do MMM")}
          </span>
        </div>
