...
import { deleteComment } from "@/app/api/comment.api";

const CommentOption = ({ data, type, postPath }: any) => {
  const { user } = useAppSelector((state) => state.auth);

  const queryClient = useQueryClient();

  const deleteComment1 = async (id: string) => {
    try {
  ...
  return (
    <>
      {user !== null && user.id === data.user.id ? (
            ...
            <>
              <DropdownMenu aria-label="Static Actions comment">
                ...
                <DropdownItem
                  ...
                  color="danger"
                  onClick={() => deleteComment1(data.id)}
                >
                  Delete comment
                </DropdownItem>
              </DropdownMenu>
