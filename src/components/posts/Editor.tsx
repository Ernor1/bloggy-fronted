...
import { getAllTags, getTagsByPostId } from "@/app/api/tag.api";
import { getAllCategories, getCategoriesByPostId } from "@/app/api/category.api";
...
const Editor = ({ post }: { post: TPost | null }) => {
  ...
  useEffect(()=>{
    ...
    if(post?.id){
      getCategoriesByPostId(post.id).then((res) => {
        setSelectedCategory(res.map((category:any)=>category.id))
      })
      getTagsByPostId(post.id).then((res) => {
        setSelectedTag(res.map((tag:any)=>tag.id))
      })
    }
